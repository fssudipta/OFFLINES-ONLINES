...
void move_and_shift(ArrayList *list, int position)
{
    if (list->current >= list->size || position < 0 || position >= list->size)
    {
    ...
    printf("Moved and shifted %d to position %d.\n", value, position);
    print_list(list);
}
...
void discard(ArrayList *list)
{
    if (list->current < list->size)
    {
        ...
        list->size--;
        printf("Discarded book with ID %d.\n", value);
        print_list(list);
    }
    else
    {
