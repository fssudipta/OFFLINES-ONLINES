...
void skip(LinkedList *list)
{
    ...
    printf("Skipped to value: %d\n", list->current->data);
}

void swap_with(LinkedList *list, int position)
{
    if (list->size == 0 || list->current == NULL || position < 0 || position >= list->size)
    {
    ...
    if (temp != NULL)
    {
        int tempData = list->current->data;
        list->current->data = temp->data;
        temp->data = tempData;
        printf("Swapped current value %d with value at position %d: %d\n", list->current->data, position, temp->data);
    }
}
...
void discard(LinkedList *list)
{
    if (list->size == 0 || list->current == NULL)
    {
        printf("List is empty or no current element to discard.\n");
        return;
    }
    int removedValue = remove_at_current(list);
    printf("Discarded value: %d\n", removedValue);
}
